...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        console.log(`> ${interaction.user.username} executed command /${interaction.commandName}`);
    } catch (error) {
        console.error(error);
                ...
                if (contentLower.includes(phrase.toLowerCase())) {
                    ...
                    if (command) {
                        try {
                            ...
                            console.log(`> Automod replied for ${message.author.username} using command /${commandName}`);
                        } catch (error) {
                            console.error(error);
                            console.log(`> Automod cannot respond to reply for ${message.author.username} using command /${commandName}`);
                        }
                        return; 
                    }
                }
